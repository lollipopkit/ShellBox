...
| ARM64 scheme | `iSH-ARM64` |
| ARM64 target | `iSH-ARM64` |
| Local Meson | `.venv/bin/meson` 1.11.1 |
| Xcode 26 asset build note | `iSH-ARM64` uses `AppARM64.xcconfig`; `GuestARM64.xcconfig` excludes `x86_64` for iPhone Simulator so `actool` and generated-source phases run against the valid ARM64 simulator slice |
...
| iPhoneOS app build | iSH-ARM64 / Release / iPhoneOS 26.4 SDK | Pass | `xcodebuild -project iSH.xcodeproj -target iSH-ARM64 -configuration Release CODE_SIGNING_ALLOWED=NO build` |
| iPhone 17 simulator build | iSH-ARM64 / Release / iPhone 17 / iOS 26.4.1 Simulator | Pass | `xcodebuild -project iSH.xcodeproj -scheme iSH-ARM64 -configuration Release -destination 'platform=iOS Simulator,name=iPhone 17,OS=26.4.1' CODE_SIGNING_ALLOWED=NO build` |
| iPhone 17 simulator install/launch | iSH-ARM64 / Release / iPhone 17 / iOS 26.4.1 Simulator | Pass | `benchmark/verify_modern_iphone.sh --build-simulator --install-simulator --launch-simulator` boots the iPhone 17 simulator, installs the app bundle, launches `app.ish.iSH.arm64`, verifies the launched process stays alive, and probes the ARM64 debug server |
| iPhone 17 simulator destination discovery | iSH-ARM64 scheme | Pass | `xcodebuild -project iSH.xcodeproj -scheme iSH-ARM64 -showdestinations` lists iPhone 17, 17 Pro, 17 Pro Max, 17e, and iPhone Air simulators |
...
verifier probes `DEBUG_SERVER_URL`, which defaults to `http://127.0.0.1:1234/`.
...
APP_PATH="/path/to/iSH ARM64.app" \
BUNDLE_ID=app.ish.iSH.arm64 \
...
```sh
xcodebuild -project iSH.xcodeproj -list
xcodebuild -project iSH.xcodeproj -scheme iSH-ARM64 -showdestinations
```
