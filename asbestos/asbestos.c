...
// Marker set to 1 on ShellBox execution threads so the signal handler can distinguish
// ShellBox threads from app threads (Swift async, networking, UI).
...
static int cpu_step_to_interrupt(struct cpu_state *cpu, struct tlb *tlb) {
    ...
    while (interrupt == INT_NONE) {
        ...
        // Linux would deliver SIGSEGV and terminate. In ShellBox the fault
