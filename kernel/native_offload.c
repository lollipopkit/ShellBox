...
static int exec_posix_spawn(const char *native_path, const char *guest_file,
    ...
    char *host_cwd = get_host_cwd();
    if (host_cwd) {
        // POSIX only standardised addchdir in Issue 8, and Apple's SDK still
        // ships just the _np spelling — the plain name compiled here only
        // because a newer local SDK happened to declare it, and failed on
        // Xcode 16.4 with an implicit-declaration error. glibc has had the _np
        // name since 2.29, so preferring it costs nothing on either.
