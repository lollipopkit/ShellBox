...
void handle_interrupt(int interrupt) {
    ...
    if (interrupt == INT_SYSCALL) {
        ...
        // adjacent allocations (no SIGSEGV). In ShellBox, mmap can leave
        ...
        if (ptr == NULL) {
            ...
            if (allow_demand_map) {
                ...
                // page that ShellBox hasn't allocated yet — the neighbor
        ...
        if (ptr == NULL) {
            ...
            // mapped) but crash in ShellBox (because we have unmapped gaps).
...
#ifdef GUEST_ARM64
        {
            ...
            // ShellBox CoW/TLB race presents stale arena metadata to musl's
            ...
            //
            // ShellBox is not a debugger host — reset the action to SIG_DFL
